		...
		<div class="page-wrapper">
            ...
            <div class="page-title-box">
                <div class="page-title-right pb-3">
                    <h2 class="page-title text-muted text-decoration-underline">Service Details</h2>
                    <div class="d-flex justify-content-end">
                        <button type="button" id="addServiceBtn" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#addServiceModal"><i class="lni lni-upload me-1"></i>Add Service</button>
                    </div>
                </div>
            </div>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table id="example2" class="table table-striped table-bordered">
                                ...
                                <td><?php echo date('d M Y', strtotime($service_date)); ?></td>
                                <td>
                                    <?php
                                    if ($return_date === "0000-00-00" || empty($return_date)) {
                                        echo "Not Received";
                                    } else {
                                ...
                                <td class="text-end">â‚¹ <?php echo !empty($amount) ? number_format($amount) : '0'; ?></td>
                                <td>
                                    <button class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#viewServiceModal" onclick="goViewService(<?php echo $service_id; ?>);" ><i class="lni lni-eye"></i></button>
                                    <button type="button" class="btn btn-sm btn-outline-warning" onclick="goEditService(<?php echo $service_id; ?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="lni lni-pencil"></i></button>
                                    <button class="btn btn-sm btn-outline-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" onclick="goDeleteService(<?php echo $service_id; ?>);"><i class="lni lni-trash"></i></button>
                                </td>
                                </tr>
                    <?php
                 } 
    ...
    $('#subCatNameEdit').on('change', function() {
        ...
        if (categoryId != '') {
            $.ajax({
                url: 'action/actService.php',
                method: 'POST',
                data: { category_editId: categoryId, serId: serId },
                dataType: 'json',
                success: function(response) {
...
function goViewService(serviceId) {
    ...
    $.ajax({
        ...
        success: function(response) {
                ...
                $('#viewAmount').text('â‚¹ ' + (response.amount ? parseFloat(response.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '0.00'));
        },
        error: function(xhr, status, error) {
            console.error('AJAX request failed:', status, error);
        }
