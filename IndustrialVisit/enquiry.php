                ...
                <div class="page-title-box">
                    <div class="page-title-right pb-3">
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="button" id="addEnquiryBtn" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#addEnquiryModal"><i class="lni lni-upload me-1"></i>Add Enquiry</button>
                        </div>
                    </div>
                </div>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table id="example2" class="table table-striped table-bordered">
                                ...
                                $enquiry_date   = htmlspecialchars($row['enquiry_date'], ENT_QUOTES);
                                $description    = htmlspecialchars($row['description'], ENT_QUOTES);

                                ?>
                                <tr>
                                    <td class="text-center align-middle" style="width: 5%;"><?php echo $i; $i++; ?></td>
                                    <td class="text-center align-middle" style="width: 15%;"><?php echo $college_name; ?></td>
                                    <td class="text-center align-middle" style="width: 10%;"><?php echo $phone_number; ?></td>
                                    <td class="text-center align-middle" style="width: 20%;"><?php echo $email; ?></td>
                                    <td class="text-center align-middle" style="width: 10%;"><?php echo date("d M Y", strtotime($enquiry_date)); ?></td>
                                    <td style="width: 30%;"><?php echo $description; ?></td>
                                    <td class="text-center align-middle" style="width: 10%;">
                                        <button type="button" class="btn btn-sm btn-outline-warning" onclick="goEditEnquiry(<?php echo $id; ?>);" data-bs-toggle="tooltip" title="Edit Enquiry" data-bs-target="#top"><i class="lni lni-pencil"></i></button>
                                        <button class="btn btn-sm btn-outline-danger" data-bs-toggle="tooltip" title="Delete Enquiry" data-bs-target="#top" onclick="goDeleteEnquiry(<?php echo $id; ?>);"><i class="lni lni-trash"></i></button>
                                    </td>
                                </tr>
                    <?php
                 } 
...
$(document).ready(function() {
    ...
    // Handle the form submission via AJAX for Add Enquiry
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'An error occurred while adding Enquiry details.'
                });
                $('#submitFormBtn').prop('disabled', false);
            }
        });
    ...
    // Handle the form submission via AJAX for Edit Enquiry
    $('#enquiryFormEdit').off('submit').on('submit', function (e) {
        e.preventDefault(); 
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'An error occurred while updating Enquiry details.'
                });
                $('#submitEditBtn').prop('disabled', false);
            }
        });
...
//Function to handle the deletion of a Enquiry.
function goDeleteEnquiry(enqId) {
    Swal.fire({
        title: 'Are you sure?',
