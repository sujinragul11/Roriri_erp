...
if (isset($_POST['htnName']) && $_POST['htnName'] == 'addClient') {
    $clientName = $_POST['clientName'] ?? '';
    $phone    = $_POST['phone'] ?? '';
    $email    = $_POST['email'] ?? '';
    $location = $_POST['location'] ?? '';
    $username = $_POST['username'] ?? ''; // Verify this exists in the form
    $password = $_POST['password'] ?? '';
    $user     = $_SESSION['id'];

    // Check if the username already exists
    $checkQuery = "SELECT `id` FROM `iv_client_tbl` WHERE `username` = '$username'";
    $checkResult = $conn->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        // Username already exists, return error
        $response['success'] = false;
        $response['message'] = "The username '$username' already exists. Please choose a different username.";
    } else {
        // Username is unique, proceed with insertion
        $insQuery = "CALL ManageClient('INSERT', NULL, '$clientName', '$phone', '$email', '$location', '$username', '$password', 'Active','$user');";
                                    
        if ($conn->query($insQuery) === TRUE) {
            $clientDir = '../../../asset/ERP/ERP_image/IV/' . $username; 

            if (!file_exists($clientDir)) {
                if (mkdir($clientDir, 0755, true)) {
                    $response['success'] = true;
                    $response['message'] = "Client details added successfully and folder created!";
                } else {
                    $response['success'] = false;
                    $response['message'] = "Client added, but folder creation failed!";
                }
            } else {
                $response['success'] = true;
                $response['message'] = "Client details added successfully! Folder already exists.";
            }
        } else {
            $response['success'] = false;
            $response['message'] = "Unexpected error in adding Client details! " . $conn->error;
        }
    }
 ...
 // edit function --------------
    if (isset($_POST['clientId_edit']) && $_POST['clientId_edit'] !== '') {
    
        $id= $_POST['clientId_edit'];
        
      $query = "CALL ManageClient('SELECT', '$id', NULL, NULL, NULL, NULL, NULL, NULL, 'Active',NULL);"; 
 ...
 if (isset($_POST['htnName']) && $_POST['htnName'] == 'editClient') {
        
    $clientId = $_POST['clientId'] ?? '';     
    $clientNameEdit = $_POST['clientNameEdit'] ?? '';
    $phoneEdit = $_POST['phoneEdit'] ?? '';
    $emailEdit = $_POST['emailEdit'] ?? '';
    $locationEdit = $_POST['locationEdit'] ?? '';
    $passwordEdit = $_POST['passwordEdit'] ?? ''; // Verify this exists in the form
  $user_edit=1;

      $updateQuery = "CALL ManageClient('UPDATE', '$clientId', '$clientNameEdit', '$phoneEdit', '$emailEdit', '$locationEdit', NULL, '$passwordEdit', 'Active','$user_edit');";
        ...
        if ($conn->query($updateQuery) === TRUE) {
            ...
            $response['message'] = "Unexpected error in Updated Client details! " . $conn->error;
        }
 ...
 if (isset($_POST['clientId_view']) && $_POST['clientId_view'] !== '') {
    $clientId = $_POST['clientId_view'];
    ...
    $query = "SELECT
            ...
            FROM
                `iv_details_tbl`
            WHERE
                `status` = 'Active' AND `college_id` = '$clientId'";  
    ...
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $clients[] = array(
                'date' => ($row['iv_date'] === '0000-00-00') ? '' : date('d M Y', strtotime($row['iv_date'])), 
                'department' => $row['department'],
                'studentCount' => $row['stu_count'],
                'staffCount' => $row['staff_count'],
                'inChargeStaff' => $row['incharge_name'],
                'ivStatus' => $row['iv_status'],
                'iv_id' => $row['iv_id'] 
            );
        }
    }
