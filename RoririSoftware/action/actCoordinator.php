...
if (isset($_POST['hdnAction']) && $_POST['hdnAction'] == 'addDepartment') {
    ...
    $updateQuery = "UPDATE `coordinator` 
                    SET `roles_res` = '$description' , `log` = '$updatedCoordinatorsJson'
                    WHERE `id` = '$dept_id'";
...
if (isset($_POST['viewId']) && $_POST['viewId'] != '') {
    ...
    if ($resultView->num_rows > 0) {
        ...
        $name = $row['name'];
        $log = json_decode($row['log'], true); // Decode the JSON log data
        $roles_res = htmlspecialchars_decode($row['roles_res'], ENT_QUOTES); // Fetch the roles_res data
...
if (isset($_POST['id']) && isset($_POST['coorId'])) {
    ...
    $queryReport = "SELECT
                    ...
                    FROM
                        `coordinator_report`
                    WHERE
                        `coordinator_id` = ? AND `dept_id` = ? AND `status` = 'Active'
                    ORDER BY
                        `created_at` DESC";

    if ($stmt = $conn->prepare($queryReport)) {
        $stmt->bind_param('ii', $coorId, $id); 
        $stmt->execute(); 
        $result = $stmt->get_result(); 
