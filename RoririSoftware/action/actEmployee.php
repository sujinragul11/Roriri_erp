...
//  coordinator work update-----

 if (isset($_POST['hdnAction']) && $_POST['hdnAction'] == 'addWork') {
    
    
    $coordinatorId = $_POST['coordinatorId'] ?? '';
    $deptId = $_POST['deptId'] ?? '';
    $assigned_id = $_SESSION['id'] ?? 0;
    $completedDate = $_POST['workDate'];
    $assignedDate = date('Y-m-d');
    $workDescription = $_POST['workDescription'];

       // Check if completed date is '0000-00-00' and handle accordingly
if (isset($completedDate)) {
                
                // Insert both assigned and completed dates
    $insQuery = "INSERT INTO `coordinator_report`(
                    `coordinator_id`,
                    `dept_id`,
                    `completed_date`,
                    `report`,
    ...
    $insQuery = "INSERT INTO `coordinator_report`(
        ...
        if ($conn->query($insQuery) === TRUE) {
            $response['success'] = true;
            $response['message'] = "Work details added successfully!";
        } else {
            $response['message'] = "Unexpected error in adding Work details! " . $conn->error;
        }
 ...
 if (isset($_GET['recordId']) && $_GET['recordId'] != '') {
    ...
    $empFetch="SELECT `rep_id`, `coordinator_id`, `dept_id`, `assigned_date`, `report`,`assign_by`, `completed_date`, `work_ststus` FROM `coordinator_report` WHERE rep_id = '$id'";
    $fetchResult = mysqli_query($conn, $empFetch);
 ...
 if (isset($_POST['hdnAction']) && $_POST['hdnAction'] == 'editWork') {
    
    $reportId = $_POST['record_edit_id'] ?? '';
    $status = $_POST['statusEdit'] ?? '';
    $deptId = $_POST['deptIdEdit'] ?? '';
    $completedDate = $_POST['workDateEdit'] ?? 0000-00-00;
    $workDescription = $_POST['workDescriptionEdit'];
    
    

  
        // Update with completed date
        $updQuery = "UPDATE `coordinator_report`
                    SET 
                        `dept_id` = '$deptId',
    ...
    if ($conn->query($updQuery) === TRUE) {
        $response['success'] = true;
        $response['message'] = "Work details updated successfully!";
    } else {
        $response['message'] = "Unexpected error in updating Work details! " . $conn->error;
    }
