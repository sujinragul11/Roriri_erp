...
//add task---
if (isset($_POST['taskName_select']) && isset($_POST['subcategory_select'])) {
    // Sanitize and validate the input
    $subcategoryId = intval($_POST['subcategory_select']); // Convert to integer to prevent SQL injection
    $taskName = trim($_POST['taskName_select']);
    $createdBy = $_SESSION['id']; // Assuming the user ID is stored in the session
    ...
    // Prepare the SQL insert query
    $query = "INSERT INTO employee_task (subcategory_id, task, created_by) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($query);
    ...
    $stmt->bind_param("isi", $subcategoryId, $taskName, $createdBy);

    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'taskName' => $taskName, 'taskId' => $stmt->insert_id]);
    } else {
        echo json_encode(['success' => false, 'message' => 'Failed to add task.']);
    }
 ...
 if (isset($_POST['action']) && $_POST['action'] === 'getTasks') {
     ...
     if (isset($_POST['action']) && $_POST['action'] === 'getCategorys') {
...
if (isset($_POST['category_table']) && $_POST['category_table'] === 'getCategory') {
