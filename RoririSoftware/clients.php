		...
		<div class="page-wrapper">
            ...
            <div class="page-title-box">
                ...
                <div class="page-title-right">
                    ...
                    <button type="button" id="addClientBtn" class="btn btn-primary position-absolute top-0 end-0" data-bs-toggle="modal" data-bs-target="#addClientModal"><i class="lni lni-plus"></i>New Clients</button>
                    </div>
        ...
        $(document).ready(function () {
                ...
                $('#addClient').off('submit').on('submit', function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                var credHtml = response.username ?
                                    '<br><br><b>Client Login Credentials:</b><br><span style="color:#0d6efd;">Username:</span> <code>' + response.username + '</code><br><span style="color:#0d6efd;">Password:</span> <code>' + response.password + '</code><br><small>(Share these with the client to access their dashboard)</small>'
                                    : '';
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Success',
                                    html: response.message + credHtml,
                                    timer: 4000
                                }).then(function () {
...
$('#updateBtn').click(function(e) {
        ...
        isValid &= validateField('UsernameE', 'usernameErrorE');
        isValid &= validateField('PasswordE', 'passwordErrorE');
                ...
                $('#editClient').off('submit').on('submit', function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                var credHtml = response.username ?
                                    '<br><br><b>Updated Login Credentials:</b><br><span style="color:#0d6efd;">Username:</span> <code>' + response.username + '</code><br><span style="color:#0d6efd;">Password:</span> <code>' + response.password + '</code><br><small>(Client must now use these credentials to login)</small>'
                                    : '';
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Updated',
