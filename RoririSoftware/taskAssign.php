...
.select2-container--bootstrap-5 .select2-selection {
    max-height: 150px; /* Adjust height to match Bootstrap 5 styles */
		...
		<div class="page-wrapper">
            ...
            <div class="page-title-box">
              ...
              ?>
                    ...
                    <button type="button" id="addEnquireBtn" class="btn btn-primary position-absolute top-0 end-0" data-bs-toggle="modal" data-bs-target="#addReportModal"><i class="lni lni-plus"></i>New Task Assign</button>
        ...
        <div class="col-md-3">
         ...
         <?php
              ...
              ?>
                        ...
                        <div class="col-md-3">
                            <label for="empName">Assign By :</label>
    ...
    <select id="statusFilter" class="form-control">
        ...
        <div class="col-md-3 mt-3">
            ...
            <button type="button" id="clearFilter" class="btn btn-secondary ms-2">
                ...
                <!--				<tfoot>-->
                <!--    <tr>-->
                <!--        <th colspan="10" style="text-align:right">Total Assigning Hours:</th>-->
...
$('#task').select2({
    ...
    createTag: function (params) {
        var term = $.trim(params.term); // Get the typed term
        if (term === '') {
        ...
        return {
            id: term,
            text: term,
             newOption: true // Flag to identify new options
        };
    }
});
   ...
   $('#task').on('select2:select', function (e) {
    let subcategory = $('#subcategory').val(); // Corrected line
    let data = e.params.data;
    // alert(data.newOption);<div class="alert-container"></div>

    if (data.newOption) {
        // If it's a new option, send an AJAX request to create the task
        $.ajax({
            url: 'action/actDailyReport.php',
            method: 'POST',
            dataType: 'json',
            data: { 
                taskName_select: data.text,
                subcategory_select: subcategory
            },
            success: function (response) {
                if (response.success) {
                    // Update the option with the new task ID
                    let newOption = new Option(response.taskName, response.taskId, true, true);
                    
                    $('#task').append(newOption).trigger('change');
                   // Show the success message using Bootstrap alert
                    let successAlert = `
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            <strong>Success!</strong> New task created successfully.
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>`;
                    // Append the alert to a container (e.g., .alert-container) on the page
                    $('.alert-container').html(successAlert);
                } else {
                   // Show the failure message using Bootstrap alert
                    let errorAlert = `
                        <div class="alert alert-danger alert-dismissible fade show" role="alert">
                            <strong>Error!</strong> Failed to create a new task. Please try again.
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>`;
            ...
            error: function () {
                // Show the error alert in case of AJAX failure
                let ajaxErrorAlert = `
                    <div class="alert alert-danger alert-dismissible fade show" role="alert">
                        <strong>Error!</strong> There was an issue with the request. Please try again.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>`;
                $('.alert-container').html(ajaxErrorAlert);
            }
        });
    }
});
     ...
     $( '#employee' ).select2( {
        ...
        $('#employee').on('select2:select', function (e) {
            ...
            if (selectedValue === "select_all") {
                ...
                $('#employee').val(allValues).trigger('change'); // Update the selection
            }
        });

        // Prevent unselecting all when "Select All" is clicked
        $('#employee').on('select2:unselect', function (e) {
            const unselectedValue = e.params.data.id;
            if (unselectedValue === "select_all") {
                $('#employee').val([]).trigger('change'); // Clear selection
            }
        });
...
$(document).ready(function() {
    ...
    { data: "name", orderable: true, className: "col-1" }, // 2
    ...
    { data: "action", orderable: true, className: "col-1" } // 8
