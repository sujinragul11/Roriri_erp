			...
			<div class="modal-content">
					<div class="modal-header">
						...
						<div class="card-body p-4">
								...
								<form class="row g-3 needs-validation" name="frmEditEmployee" id="editEmployee" enctype="multipart/form-data" novalidate>
...
<div class="modal fade" id="editWorkModal" tabindex="-1" aria-labelledby="addEmployeeModalLabel" aria-modal="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="card-body p-4">
                <form class="row g-3 needs-validation" id="editWork" novalidate>
                    <input type="hidden" name="hdnAction" value="editWork">
                    <input type="hidden" name="record_edit_id" id="record_edit_id">

                    <!-- Department Selection -->
                    <div class="col-md-12">
                        <div class="mb-4">
                            <label for="deptIdEdit" class="form-label">Department</label>
                            <select class="form-select" name="deptIdEdit" id="deptIdEdit" required>
                                <option value="">---Choose a Coordinator---</option>
                                <?php
                                $corQuery = "SELECT id, name FROM coordinator WHERE JSON_CONTAINS(log, JSON_OBJECT('id', '$empId', 'status', 'active'))";
                                $resultCor = $conn->query($corQuery);

                                if ($resultCor) {
                                    if ($resultCor->num_rows > 0) {
                                        while ($row = mysqli_fetch_array($resultCor, MYSQLI_ASSOC)) {
                                            echo "<option value='" . htmlspecialchars($row['id']) . "'>" . htmlspecialchars($row['name']) . " Coordinator</option>";
                                        }
                                    } else {
                    ...
                    <div class="col-md-12">
                        <label for="workDescriptionEdit" class="form-label">Work Description <span class="text-danger">*</span></label>
                        <div id="quillEditorEdit" style="height: 150px;"></div>
                        <textarea name="workDescriptionEdit" id="workDescriptionEdit" style="display: none;"></textarea>
                        <div class="invalid-feedback">Please enter a description of the work.</div>
                    </div>

                    <!-- Status Field -->
                    <div class="col-md-12">
                        <label for="statusEdit" class="form-label">Status <span class="text-danger">*</span></label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="submit" id="submitBtnWorkEdit" class="btn btn-primary">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
